...
const noop = function() {}
const colors = ['blue', 'green', 'cyan', 'yellow', 'red', 'magenta']
...
Object.assign(Chastifol.prototype, {
  ...
  start() {
    ...
    for (let i = 0; i < commandsLength; i++) {
      this.startProcess(i)
      let waitForExit = this.attachToProcess(i)
      processRuns.push(waitForExit)
    }
  ...
  // initialies and starts the child process
  startProcess(commandIndex) {
    let command = this.commands[commandIndex]
    log(`executing command: ${command}`)
    let cp = ChildProcess.exec(command, {}, noop)
  ...
  // attach additional hooks to process, and wait for exit
  attachToProcess(commandIndex) {
    let command = this.commands[commandIndex]
    let cp = this.childProcesses[commandIndex]
    let colorCurrent = !!this.color[commandIndex]
